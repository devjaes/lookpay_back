...
public interface CDCardRepository extends JpaRepository<CDCardModel, Long> {
    ...
    @Query(value = "SELECT cd.id, cd.balance, cd.card_holder_name, cd.card_type, cd.ccv, cd.expiration_date, cd.\"number\" FROM cd_cards cd inner join payment_methods p on p.cd_card_id = cd.id WHERE p.client_id =:clientId", nativeQuery = true)
    List<CDCardModel> findAllByClientId(@Param("clientId") Long clientId);

    @Query(value = "SELECT cd.id, cd.balance, cd.card_holder_name, cd.card_type, cd.ccv, cd.expiration_date, cd.\"number\" FROM cd_cards cd inner join payment_methods p on p.cd_card_id = cd.id WHERE p.client_id =:clientId and cd.card_type = 'CREDIT_CARD'", nativeQuery = true)
