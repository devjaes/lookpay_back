...
public interface UserRepository extends JpaRepository<UserModel, Long> {
    ...
    @Query(value = "select u.id as id, email, u.username as name, password, phone_number, address, rol, c.name as city, p.name as province, c.id as city_id from users u inner join cities c on c.id = u.city_id inner join provinces p on p.id = c.province_id where u.id=:userId", nativeQuery = true)
